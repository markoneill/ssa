...
void run_sockops_tests(void) {
	...
	if (getsockopt(sock_fd, IPPROTO_IP, TLS_REMOTE_HOSTNAME, hostname_retrieved, &hostname_length) == -1) {
		perror("getsockopt: TLS_REMOTE_HOSTNAME");
		exit(EXIT_FAILURE);
	}
...
int connect_to_host(char* host, char* service) {
	...
	for (addr_ptr = addr_list; addr_ptr != NULL; addr_ptr = addr_ptr->ai_next) {
		...
		if (sock == -1) {
	        ...
	        if (setsockopt(sock, IPPROTO_IP, TLS_REMOTE_HOSTNAME, host, strlen(host)+1) == -1) {
			perror("setsockopt: TLS_REMOTE_HOSTNAME");
			close(sock);
			continue;
		}
